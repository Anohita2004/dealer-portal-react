...
// Send location every 11 seconds (respects 10s rate limit)
console.log('🚚 Starting simulation...');
console.log('⚠️  Rate limit: 10 seconds per update');
...
```
🚚 Starting simulation...
⚠️  Rate limit: 10 seconds per update
📍 Open Live Tracking page to see movement!
✅ Location sent: (22.540738, 88.353908)
✅ Location sent: (22.540838, 88.354008)
...
```
...
- Updates every 11 seconds (respects 10s rate limit)
...
5. Rate limit errors - if you see 429 errors, increase interval to 11+ seconds

---

## Rate Limiting

⚠️ **Important**: The backend enforces a rate limit of **1 update per 10 seconds** per truck.

- ✅ **Correct**: Send updates every 11 seconds (or more)
- ❌ **Wrong**: Sending every 3 seconds will cause 429 errors

The mobile app (`driverLocationService.js`) automatically handles rate limiting:
- Queues updates that arrive too frequently
- Retries failed updates after rate limit period
...
- **Solution**: Increase interval between updates to 11+ seconds
- The mobile app handles this automatically, but test scripts need manual adjustment
...
- Verify backend is emitting `truck:location:update` events
...
5. **Watch the web map** - truck should update every 10 seconds (rate limited)

The mobile app automatically:
- ✅ Respects 10-second rate limit
- ✅ Queues updates that arrive too frequently
...
- **Always use 11+ second intervals** to avoid rate limit errors
...
- Monitor console for rate limit errors (429) - if you see them, increase interval
...
- **Body**: `{ truckId, lat, lng, speed?, heading?, timestamp }`
- **Rate Limit**: 1 request per 10 seconds per truck

### Socket.IO Events
- **Emit**: `track_truck` with `{ truckId }`
- **Listen**: `truck:location:update` with location data
- **Event Data**: `{ truckId, lat, lng, speed?, heading?, timestamp, assignmentId? }`

### Map Components
- `TrackingMap.jsx` - Shows all active trucks
- `LiveTrackingMap.jsx` - Shows single order tracking
- `TruckLocationMap.jsx` - Shows trucks filtered by driver phone

All components use `UpdatingMarker` which smoothly updates marker positions when location changes.
