...
global.FileReader = class FileReader {
  ...
  beforeEach(() => {
    ...
    const { orderAPI, invoiceAPI } = require('../../services/api');
    ...
    it.skip('should create invoice from approved order', async () => {
      const { orderAPI, invoiceAPI, materialAPI } = require('../../services/api');
      ...
      if (!raiseInvoiceButton.disabled) {
        ...
        await waitFor(() => {
          expect(invAPI.createInvoice).toHaveBeenCalled();
        }, { timeout: 3000 });
      } else {
  ...
  describe('Dealer Staff Payment Initiation', () => {
    ...
    it.skip('should create payment request from invoice', async () => {
      ...
      // Wait for invoice dropdown - first native select in the form
      const [invoiceSelect] = await screen.findAllByRole('combobox', undefined, {
        timeout: 5000,
      });
      ...
      const paymentModeSelect = screen.getByLabelText(/payment mode/i);
